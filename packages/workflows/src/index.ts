...
const workflows: readonly WorkflowDefinition[] = [
  ...
  },
  {
    id: "remediation-pr",
    version: 1,
    displayName: "Remediation PR",
    description: "Prepare a bounded repository-only remediation patch set for explicit human approval before any branch, commit, or pull-request write.",
    reviewInstructions: "Bind every proposed change to deterministic finding evidence and the exact scanned commit. Keep changes minimal and repository-relative, include focused regression tests where appropriate, and never treat proposal generation as approval. Do not delete files, modify .git metadata, widen to unrelated findings, access live targets, or perform repository writes until an exact patch-set approval is supplied.",
    categories: "all",
    capabilities: [
      "read-normalized-findings",
      "read-repository-inventory",
