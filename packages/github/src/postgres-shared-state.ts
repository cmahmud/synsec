  ...
  async claim(deliveryIdValue: string): Promise<GitHubWebhookDeliveryClaim> {
    ...
    const result = await this.pool.query(
      `WITH claimed AS (
        ...
        WHERE synsec_github_replay.received_at <= clock_timestamp() - ($2::bigint * interval '1 millisecond')
        RETURNING received_at
      )
      SELECT true AS accepted, received_at FROM claimed
      UNION ALL
      SELECT false AS accepted, received_at FROM synsec_github_replay
      WHERE delivery_id = $1 AND NOT EXISTS (SELECT 1 FROM claimed)
      LIMIT 1`,
      [id, this.retentionMs],
    );
    const row = result.rows[0];
    if (!row) throw new Error("PostgreSQL replay claim did not return durable state.");
    return {
      accepted: row.accepted === true,
      deliveryId: id,
      receivedAt: timestamp(row.received_at, "GitHub webhook replay receivedAt"),
    };
  }
