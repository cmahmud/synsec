...
async function enqueueFailed(queue, input, modifiedAt) {
  ...
  await queue.fail(job.jobId, leased.leaseId);
...
test("retention deletes only failed jobs whose terminal record is old", async () => {
  ...
  const oldFailed = await enqueueFailed(queue, {
    deliveryId: "old-failed", installationId: 1, repository: "o/old", headSha: "a".repeat(40), event: "push",
    createdAt: new Date(now - 12 * 60 * 60 * 1000).toISOString(),
  }, now - 2 * 60 * 60 * 1000);
  const recentFailed = await enqueueFailed(queue, {
    deliveryId: "recent-failed", installationId: 1, repository: "o/recent", headSha: "b".repeat(40), event: "push",
    createdAt: new Date(now - 12 * 60 * 60 * 1000).toISOString(),
  }, now - 30 * 60 * 1000);
  await queue.enqueue({
    deliveryId: "old-pending", installationId: 1, repository: "o/pending", headSha: "c".repeat(40), event: "push",
    createdAt: new Date(now - 24 * 60 * 60 * 1000).toISOString(),
  });

  const result = await pruneGitHubAppFailedJobs(queue, { now: () => now, failedJobRetentionMs: 60 * 60 * 1000 });
...
test("retention caps deletions per maintenance pass", async () => {
  ...
  for (let index = 0; index < 3; index += 1) {
    await enqueueFailed(queue, {
      deliveryId: `failed-${index}`, installationId: 2, repository: `o/r${index}`,
      headSha: `${index + 1}`.repeat(40), event: "push",
    }, now - 3 * 60 * 60 * 1000 - index);
  }
  const result = await pruneGitHubAppFailedJobs(queue, { now: () => now, failedJobRetentionMs: 60 * 60 * 1000, maxDeletes: 2 });
...
test("retention rejects unbounded policy values", async () => {
  const now = Date.parse("2026-08-22T20:00:00.000Z");
  const queue = await setup(now);
  await assert.rejects(() => pruneGitHubAppFailedJobs(queue, { failedJobRetentionMs: 1 }), /retention must be between/);
  await assert.rejects(() => pruneGitHubAppFailedJobs(queue, { maxDeletes: 1001 }), /maxDeletes must be between/);
});
