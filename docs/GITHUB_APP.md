...
`@synsec/github/app-handler` composes signature verification, replay claiming, installation-state synchronization, durable authorization, and queue dispatch in one tested boundary. Duplicate authenticated deliveries do not mutate installation state or enqueue duplicate work; installation-management events remain bookkeeping-only.
...
3. production composition from `runNextGitHubAppScanJob()` into `runScanEngine()` and the existing Checks/SARIF publishers;
